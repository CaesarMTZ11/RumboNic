...
import { createDemoGeneratedRoute } from "@/src/features/route-planner/data/generatedRouteData";
import { Alert, Pressable, StyleSheet, Text, View } from "react-native";

import { AppButton, AppInput, ScreenContainer } from "@/src/components/common";
import {
    FormSection,
    OptionCard,
    PeopleStepper,
...
export default function RouteGeneratorScreen() {
    ...
    const routeParams = useLocalSearchParams<{
        preferredPlaceId?: string | string[];
        preferredPlaceName?: string | string[];
    }>();

    const preferredPlaceName = Array.isArray(
        routeParams.preferredPlaceName,
    )
        ? routeParams.preferredPlaceName[0]
        : routeParams.preferredPlaceName;

    const setPreferences = useRoutePlannerStore((state) => state.setPreferences);

    const setGeneratedRoute = useRoutePlannerStore(
        (state) => state.setGeneratedRoute,
    );
    ...
    function toggleInterest(currentValues: string[], interest: string) {
        if (currentValues.includes(interest)) {
            return currentValues.filter((value) => value !== interest);
        }
        ...
        <ScreenContainer contentContainerStyle={styles.container}>
            <View style={styles.topBar}>
                ...
                <View style={styles.topBarText}>
                    <Text style={styles.topBarTitle}>Generar ruta</Text>

                    <Text style={styles.topBarSubtitle}>Personalizada con IA</Text>
                </View>
            ...
            <View style={styles.hero}>
                <View style={styles.heroIcon}>
                    <Ionicons name="sparkles" size={27} color={colors.warning} />
                </View>

                <Text style={styles.heroTitle}>Diseñemos tu próxima aventura</Text>

                <Text style={styles.heroDescription}>
                    Cuéntanos qué buscas y RumboNic preparará un itinerario adaptado a ti.
                </Text>
            </View>

            {preferredPlaceName ? (
                <View style={styles.preferredPlaceCard}>
                    <View style={styles.preferredPlaceIcon}>
                        <Ionicons
                            name="location"
                            size={20}
                    ...
                    <View style={styles.preferredPlaceContent}>
                        <Text style={styles.preferredPlaceLabel}>
                            LUGAR SELECCIONADO
                        </Text>

                        <Text style={styles.preferredPlaceName}>
                            {preferredPlaceName}
                        </Text>

                        <Text style={styles.preferredPlaceDescription}>
                            RumboNic lo tomará como referencia para generar
                            tu itinerario.
                        </Text>
                    </View>
                </View>
            ...
            >
                <View style={styles.budgetCard}>
                    ...
                    <View style={styles.budgetRange}>
                        ...
                        <Text style={styles.rangeText}>C$ 10,000</Text>
                    </View>
                </View>
            </FormSection>

            <FormSection number={4} title="¿Cuántas personas viajan?">
                <Controller
                    control={control}
                    name="people"
                    render={({ field }) => (
                        <PeopleStepper value={field.value} onChange={field.onChange} />
                    )}
                />
            </FormSection>

            <FormSection number={5} title="¿Con quién viajas?">
                <Controller
                    control={control}
                    name="companion"
                    render={({ field }) => (
                            ...
                            <View style={styles.chipsContainer}>
                                {interestOptions.map((interest) => (
                                    <SelectionChip
                                        ...
                                        icon={interest.icon}
                                        selected={field.value.includes(interest.value)}
                                        onPress={() =>
                                            field.onChange(
                                                toggleInterest(field.value, interest.value),
                                            )
                                        }
                                    />
                                ))}
            ...
            <View style={styles.aiNotice}>
                ...
                <Text style={styles.aiNoticeText}>
                    La IA utilizará lugares registrados y verificados por RumboNic para
                    evitar recomendaciones inexistentes.
                </Text>
            </View>
